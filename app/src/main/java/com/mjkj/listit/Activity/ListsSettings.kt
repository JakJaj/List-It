...
class ListsSettings : ComponentActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent{
            ...
            var listDescription = remember { mutableStateOf("")}
            var listColor = remember{ mutableStateOf("") }
            ...
            if(loading.value){
                ...
                Surface (modifier = Modifier.fillMaxSize(),
                    ...
                    Scaffold( //TODO: ZMIEN TO NA TOP APP BAR OD USTAWIEN
                        topBar = {
                            ListAppBar(
                        ...
                        Column(
                            ...
                            var newListName = OutlinedTextField(label = "List name max 14 characters", textIn = listName.value)
                            var newListDescription = OutlinedTextField(label = "Description", textIn = listDescription.value)
                            Spacer(modifier = Modifier.padding(10.dp))
                            Text(text = "Color:", color = MaterialTheme.colorScheme.primary, fontSize = 20.sp)
                            var newListColor = DropdownMenuBox(items = colorArray, selectedItem = listColor.value)

                            Spacer(modifier = Modifier.padding(10.dp))
                            ButtonFilled("Save") {
                                if(!(newListName != listName.value || newListDescription != listDescription.value || newListColor != listColor.value)){
                                    Toast.makeText(baseContext, "No changes", Toast.LENGTH_SHORT).show()
                                }
                                else if(newListName == ""){
                                    Toast.makeText(baseContext, "List name cannot be empty", Toast.LENGTH_SHORT).show()
                                }
                                else if(newListName.length > 14){
                                    Toast.makeText(baseContext, "List name is too long", Toast.LENGTH_SHORT).show()
                                }
                                else {
                                    ...
                                    Toast.makeText(baseContext, "List updated", Toast.LENGTH_SHORT)
                                        .show()
                                }
                            }
                            Spacer(modifier = Modifier.padding(10.dp))
                                        ...
                                        .addOnSuccessListener { document ->
                                            ...
                                            firestore.collection("lists").document(listCode).delete()
                                                .addOnSuccessListener {
                                                    ...
                                                    firestore.collection("lists").document(newListCode).set(data!!)
                                                        .addOnSuccessListener {
                                                            Log.d("ListsSettings", "Document added")
                                                            firestore.collection("lists").document(newListCode).get()
                                                                .addOnSuccessListener { document3 ->
                                                                    val data1 = document3.data
                                                                    Log.d("ListsSettings", "Data after update: $data1")

                                                                    firestore.collection("users").get()
                                                                        .addOnSuccessListener { documents ->
                                                                            for (document in documents){
                                                                                val lists = document.get("lists") as? MutableList<String>
                                                                                if (lists != null){
                                                                                    if (lists.contains(listCode)){
                                                                                        lists.remove(listCode)
                                                                                        lists.add(newListCode)
                            ...
                            Spacer(modifier = Modifier.padding(10.dp))
                            DangerFilledButton("Delete list") { //TODO: EWENTUALNE BADZ MILY I ZROB DIALOG CZY NA PEWNO
                                    ...
                                    .addOnSuccessListener {
                                        ...
                                        firestore.collection("users").get()
                                            .addOnSuccessListener { documents ->
                                                ...
                                                Toast.makeText(baseContext, "List deleted", Toast.LENGTH_SHORT)
                                                    .show()
                                                ...
                                                currentUserR.get().addOnSuccessListener { documentSnapchot ->
                                                    ...
                                                    if (documentSnapchot.exists()) {
                                                        Log.d("ListsSettings", "DocumentSnapshot data: ${documentSnapchot.data}")
                                                        val lists = documentSnapchot.get("lists") as? MutableList<String>

                                                        if (lists.isNullOrEmpty()) {
                                                            Log.d("ListsSettings", "No lists go to empty lists activity")
                                                            val intent =
                                                                Intent(this@ListsSettings, EmptyListsListActivity::class.java)
                                                            startActivity(intent)
                                                            finish()
                                                        } else {
                                                            Log.d("ListsSettings", "Lists exist go to lists activity")
                                                            val intent =
                                                                Intent(this@ListsSettings, FilledListsListActivity::class.java)
                                                            startActivity(intent)
                                                            finish()
                                                        }
                                                    } else {
                                                        Log.d("ListsSettings", "No such document")
                                                    }
